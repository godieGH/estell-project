...
const configureStorage = (uploadType) => {
  return multer.diskStorage({
    destination: (req, file, cb) => {
      ...
      switch (uploadType) {
        case 'attachment':
          destinationPath = path.join(__dirname, '..', 'uploads', 'messages', 'attachment', conversationId);
          break;
        case 'voice_note':
          destinationPath = path.join(__dirname, '..', 'uploads', 'messages', 'voice_notes', 'original', conversationId);
          break;
        case 'voice_note_m4a':
          destinationPath = path.join(__dirname, '..', 'uploads', 'messages', 'voice_notes', 'm4a', conversationId);
          break;
        case 'hls_video':
          destinationPath = path.join(__dirname, '..', 'uploads', 'messages', 'attachment', 'hls', conversationId);
          break;
        default:
      ...
      fs.mkdir(destinationPath, { recursive: true }, (err) => {
        if (err) {
          console.error(`Error creating directory ${destinationPath}: ${err.message}`);
          return cb(err);
        }
...
exports.msgUploadVoiceNote = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (file.mimetype.startsWith('audio/')) {
      ...
      console.warn(`File filter: Only audio files are allowed for voice notes - ${file.mimetype}`);
      cb(new Error('Only audio files are allowed for voice notes!'));
    }
  }
});

exports.convertAudioToM4a = async function(inputPath, outputDir, outputFileName) {
  const outputPath = path.join(outputDir, `${outputFileName}.m4a`);
  console.log(`Starting audio conversion to M4A: ${inputPath} -> ${outputPath}`);

  return new Promise((resolve, reject) => {
    fs.mkdir(outputDir, { recursive: true }, (err) => {
      if (err) {
        console.error(`Error creating output directory for M4A conversion ${outputDir}: ${err.message}`);
        return reject(new Error(`Failed to create output directory for M4A: ${err.message}`));
      }
      ...
      ffmpeg(inputPath)
        ...
        .on('error', (err) => {
          console.error(`Error during audio conversion to M4A (${inputPath}): ${err.message}`);
...
exports.convertVideoToHLS = async function(inputPath, outputDir, outputFileNamePrefix) {
  const playlistName = `${outputFileNamePrefix}.m3u8`;
  const playlistPath = path.join(outputDir, playlistName);
  console.log(`Starting video conversion to HLS: ${inputPath} -> ${playlistPath}`);

  return new Promise((resolve, reject) => {
    fs.mkdir(outputDir, { recursive: true }, (err) => {
      if (err) {
        console.error(`Error creating output directory for HLS conversion ${outputDir}: ${err.message}`);
        return reject(new Error(`Failed to create output directory for HLS: ${err.message}`));
      }
      ...
      ffmpeg(inputPath)
        ...
        .on('progress', function(progress) {
            console.log('Processing HLS: ' + progress.percent + '% done, ' + progress.timemark + ' time');
        })
        .on('end', () => {
        ...
        .on('error', (err) => {
          console.error(`Error during video conversion to HLS (${inputPath}): ${err.message}`);
...
exports.zipFile = function(filePath, outputDir, outputFileName) { // Removed 'async' from here
    return new Promise((resolve, reject) => { // Removed 'async' from the executor here
